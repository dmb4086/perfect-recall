...
|--------|------------|--------|
| **Recall@k** | % of relevant memories in top-k results | > 90% |
...
| **Answer Accuracy** | % of questions answered correctly | > 85% |
| **Continuity Score** | % of cross-session facts retained | > 80% |

### 4.2 Cost Metrics

| Metric | Definition | Target vs Baseline |
|--------|------------|-------------------|
| **Tokens per Query** | Avg tokens in context | < 30% of full-context |
| **Cost per Session** | API costs (embedding + LLM) | < 50% of full-context |
| **Storage Cost** | Per-user storage cost | < $0.01/day |
| **Compute Cost** | Embedding + retrieval cost | < 20% of LLM cost |

### 4.3 Latency Metrics

| Metric | Definition | Target |
|--------|------------|--------|
| **p50 Retrieval** | Median retrieval time | < 50ms |
| **p95 Retrieval** | 95th percentile | < 200ms |
| **p99 Retrieval** | 99th percentile | < 500ms |
...
| **Conflict Detection Rate** | % of conflicts caught | > 80% |
| **Abstention Appropriateness** | % correct abstentions | > 85% |
| **Memory Compression** | Compression ratio | 5:1 to 10:1 |
...
*Assumptions: GPT-4 pricing, 4K input, 1K output, 100 memories retrieved*
...
→ PerfectRecall is cheaper after 2 queries
```
...
| Scenario | Why Memory Wins | Expected Improvement |
|----------|-----------------|-------------------|
| **Long conversations** (> 50 turns) | Context window limits | 40% better recall |
| **Multi-session tasks** | Full-context loses everything | 80% better continuity |
| **Fact-heavy domains** | Semantic retrieval beats linear scan | 30% better accuracy |
| **Repeated queries** | Caching + learning | 50% cost reduction |
| **Temporal reasoning** | Bi-temporal model | 60% better accuracy |

### 7.2 Memory Doesn't Win

| Scenario | Why Memory Doesn't Help | Recommendation |
|----------|------------------------|----------------|
| **Short conversations** (< 10 turns) | Full-context is fine | Don't use memory |
| **Single-turn Q&A** | No history to remember | Don't use memory |
| **Real-time streaming** | Latency requirements | Simplified memory |
| **Highly unstructured** | No patterns to learn | Full-context better |
| **Security-critical** | Need audit trail | Hybrid approach |
...
|--------|--------|----------|
| DMR Recall@5 | > 90% | Critical |
| Cost reduction | > 50% vs full-context | Critical |
| Continuity score | > 80% | Critical |
| Latency p95 | < 200ms | High |
| Abstention accuracy | > 85% | High |
| Conflict detection | > 80% | Medium |
