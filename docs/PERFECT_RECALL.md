...
| "Perfect" retrieval | 85-95% accuracy on benchmarks, not 100% |
| "No information loss" | Compression and summarization lose information |
| "Instant recall" | <200ms p95 latency, not instantaneous |

**Read [HARD_PARTS.md](./HARD_PARTS.md) for the unsolved problems.**
...
|--------|--------|-------------------|--------|
| DMR Recall@5 | >85% | Standard DMR benchmark | To be measured |
| Cross-session continuity | >80% | Custom multi-session test | To be measured |
| Memory latency (p95) | <200ms | End-to-end retrieval | To be measured |
| Token reduction vs full-context | >50% | Per-query token count | To be measured |
| Abstention accuracy | >85% | Custom benchmark | To be measured |

**Note:** These are targets, not achieved values. See [BENCHMARKING.md](./BENCHMARKING.md) for measurement methodology.
...
Memory-enabled agents show improvements in:
...
See [PROCEDURAL_MEMORY.md](./PROCEDURAL_MEMORY.md) for the full redefinition.
...
See [V1_ARCHITECTURE.md](./V1_ARCHITECTURE.md) for complete algorithm specifications.

#### Memory Write Gate

```python
def should_write(content, context) -> WriteDecision:
    """
    Multi-factor decision on what to store.
    Prevents spam while preserving important information.
    """
    scores = {
        'novelty': score_novelty(content),
        'utility': score_utility(content, context),
        'explicit': score_explicit_marking(content),
    }
    
    return weighted_score(scores) >= dynamic_threshold()
```

See [HARD_PARTS.md](./HARD_PARTS.md) for the hard problems in write decisions.
...
**Success Criteria**:
- DMR Recall@5 > 70%
- Abstention accuracy > 75%
- Cost reduction > 30% vs full-context
...
**Success Criteria**:
- DMR Recall@5 > 85%
- Cost reduction > 50% vs full-context
- Cross-session continuity > 80%
...
|-----------|---------------|--------|
| **DMR** | Deep retrieval from long history | Recall@5 > 85% |
| **LongMemEval** | Temporal reasoning | Accuracy > 75% |
| **LoCoMo** | Cross-session continuity | Continuity > 80% |
| **Custom** | Write decisions, abstention | Accuracy > 85% |

See [BENCHMARKING.md](./BENCHMARKING.md) for full specification.
...
| [BENCHMARKING.md](./BENCHMARKING.md) | Measurement methodology and baselines |
...
4. Measure against real baselines ([BENCHMARKING.md](./BENCHMARKING.md))
